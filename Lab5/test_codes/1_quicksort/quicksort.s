...
main:
    xor    a3, zero, 0x200     # 指定排序问题的规模。0x200则代表要给0x200=512个数字进行快速排序。
    
    lui    sp, 0x00010         # 设置栈顶指针 sp=0x10000 
    
    xor    a0, zero, zero      # 准备函数参数，a0=0, 说明要排序的数组的RAM起始地址为0
    xor    a1, zero, zero      # 准备函数参数，a1=0，说明从第0个字开始排序
    addi   a2, a3  , -1
    slli   a2, a2  , 2         # 准备函数参数，a2=数组最后一个元素的地址偏移。我们要排0x200=512个数，最后一个数的地址为0x7fc
    
    jal    ra  , QuickSort     # 开始排序
    jal   zero, program_finish  # 排序结束
